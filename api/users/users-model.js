...
function findBy(filter) {
  /**
    ...
    [
     ...
     select user_id, useranme, password, role_name from users join roles on
      users.role_id = roles.role_id where users.user_id = 1;
   */
      return db('users')
      .join('roles', 'users.role_id', 'roles.role_id')
...
function findById(user_id) {
   ...
   */
    return db('users')
    .join('roles', 'users.role_id', 'roles.role_id')
    .select('user_id', 'username', 'role_name') // corrected here
...
async function add({ username, password, role_name }) { // done for you
  ...
  await db.transaction(async trx => {
    ...
    if (role) {
      ...
      const [role_id] = await trx('roles').insert({ role_name: role_name.trim() })
